...
* DB32 DLSE supports the following chips by Espressif Systems: ESP32-C3, ESP32-C6 & ESP32-C5
...
* The DB32 DLSE requires sufficiently powered ESP32 hardware during all of its operation\
  _&#x41;n insufficient power supply may cause unexpected behaviour and system reboots._
...
* The flight controller must send regular heartbeats to the ESP32 DLSE via the UART connection\
  _&#x42;ackground: The ESP32 DLSE detects the armed state of the flight controller via heartbeat messages. In case no valid MAVLink message is received within a period of 1000ms, the DB32 DLSE assumes that the armed state is no longer valid & unknown. A power-off command via the power management will be ignored._
...
* When operated in TRAIL mode, the ESP32 shuts down the serial connection after 10 minutes. TRAIL mode is not permitted or safe to use for operation. It is intended for testing & setup only.
...
1. DroneBridge for ESP32 DLSE may crash, reset, freeze or stop operating abruptly and without prior notice. Never rely on DLSE solely for the safe operation of your UAV. A loose contact, e.g., on the antenna connector, may cause a crash of the ESP32 firmware, requiring a reboot.\
   _&#x50;roposed mitigation action:_\
   _&#x4E;ever use DLSE as the sole communication link. Ensure a backup link is in place to allow safe operation (or safe backup mode) of your device._
2. DroneBridge for ESP32 DLSE may send incorrect power-control commands using the DLSE power management function.\
   _&#x50;roposed mitigation action:_\
   _&#x54;he power module shall not be able to cut the power to the drone while the drone is airborne. E.g. by preventing a power-off (even if commanded by the ESP32) when the current draw of the drone is higher than the idle current when on the ground._
3. DroneBridge for ESP32 may lock up internally, sending the same message continuously to the connected device.\
   _&#x50;roposed mitigation action: An additional data link should be able to override DLSE UART traffic in case the DLSE commands to the FC are compromised. E.g. this may be achieved by sending high rates of correct commands or by having a mode on the FC that does not rely on UART traffic from the DLSE._
...
* When using the Power Management Function of DLSE, it is highly recommended to use an active-low logic (low signal=power; high signal=no power) to trigger the power supply to the rest of the drone. This improves safety since a faulty ESP32 will likely pull all pins to low and thus not interrupt the power to the rest of the drone.\
  An alternative solution is to have a signal with a specific pulse length (not just a pulse in general) to trigger the power-on/-off circuit. Such a signal is also less likely to be created by a failing ESP32.
...
* Check the temperature of the ESP32 hardware during all operating modes & environmental conditions to prevent overheating. Overheating of the ESP32 may cause degradation of the radio signal due to thermal throttling of the radio or unexpected (erratic) behaviour like reboots or crashes of the system.
...
* Only use shielded ESP32 modules to reduce interference.
* The legal operation of DLSE according to the local regulations is the responsibility of the operator.\
  _&#x4E;ote: In Wi-Fi Client Mode, the DLSE will use the regulatory domain published by the access point to adjust the maximum transmission power of the ESP32._
