...
DroneBridge for ESP32 supports two radio transport options for swarm deployments. Both are fully encrypted using AES-256-GCM, but they are optimised for different operational trade-offs.
...
This makes it a strong option for outdoor or infrastructure-free deployments. The trade-off is lower data throughput and earlier link saturation as the swarm grows.
...
* All nodes must use the same password because it is used as the shared key.
